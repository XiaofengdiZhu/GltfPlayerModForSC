...
namespace Game {
    /// <summary>
    /// glTF 玩家自动跳跃组件：地面行走中前方约 ClimbTriggerDistance 处有 1 米高可碰撞障碍时，
    /// 改用攀爬动画（ClimbUp_1m_RM，root motion Override 接管位移）替代原版物理跳跃。
    /// 继承 ComponentAutoJump 复用其可爬判定，但用前瞻距离触发（替代基类的贴脸碰撞触发），
    /// 仅在触发瞬间撤销 JumpOrder 并标记攀爬待处理，由 ComponentGltfPlayerBaseController 消费标志进入 ClimbUp 相位。
    /// </summary>
    /// <remarks>
    /// 前瞻触发：基类依赖 CollisionVelocityChange（已撞障碍、贴脸）才触发，但 ClimbUp_1m_RM 动画设计为
    /// 前方约 0.8m 起步攀爬——贴脸起步会使 root motion 推模型穿模且结束位置偏远。
    /// 故本类 TryForwardClimb 用 DDA 沿 body.Forward 射线遍历穿过的格，reach（= ClimbTriggerDistance）内
    /// 找第一个可爬障碍（可碰撞、上方不可碰撞、非 NoAutoJump）即触发；命中则撤销 JumpOrder + 置攀爬标志
    /// + 设 m_lastAutoJumpTime（门控基类同帧贴脸判定）。
    ///
    /// 方向门控（climb 动画沿玩家 Forward 直线，故检测方向须 = Forward 射线）：
    /// - DDA 只查 Forward 射线实际穿过的格：朝障碍角走时射线先穿 Air 再穿障碍（reach 内→爬）；
    ///   侧向移动时射线不穿侧方障碍（不爬），避免 climb 沿 Forward 爬向空气。
    /// - 仅当有前进输入（按 W，LastWalkOrder.Y &gt; ForwardInputThreshold）才攀爬；纯侧移（A/D）走原版物理跳。
    /// - 仅地面（StandingOnValue 有值）才触发，防空中掉落误爬。
    ///
    ...
    public class ComponentGltfPlayerAutoJump : ComponentAutoJump {
        ...
        // 前进输入阈值（LastWalkOrder.Y）：超过视为按 W 前进，才考虑攀爬（climb 沿 Forward，侧向输入走物理跳）
        ...
        /// <summary>
        /// 前瞻检测触发攀爬；未命中则 base.Update 兜底（原版贴脸 AutoJump，仅 Forward 方向可爬才撤销改攀爬）。
        ...
        public override void Update(float dt) {
            ...
            // 前瞻命中：地面 + Forward 方向 + 距障碍 < ClimbTriggerDistance → 提前触发攀爬（替代基类贴脸触发）
            if (onGround
                && m_subsystemTime.GameTime - m_lastAutoJumpTime > 0.3
                && TryForwardClimb(distanceGated: true)) {
            ...
            // 兜底撤销：base 贴脸触发时（前瞻漏判），仅地面 + Forward 方向可爬才撤销改攀爬；
            // 侧向障碍（按 A/D）保留物理跳（原版行为），避免 climb 动画沿 Forward 爬向空中。
            if (after > before && after > 0f) {
                if (onGround && TryForwardClimb(distanceGated: false)) {
                    m_componentCreature.ComponentLocomotion.JumpOrder = 0f;
                    m_climbUpPending = true;
                }
            }
        ...
        /// <summary>
        /// Forward 方向可爬障碍检测：DDA 沿 body.Forward 射线遍历穿过的格，reach 内找第一个可爬障碍。
        /// </summary>
        /// <param name="distanceGated">true 要求障碍在 ClimbTriggerDistance 内（前瞻）；false 贴脸也接受（兜底撤销判定）。</param>
        /// <returns>true 表示 Forward 方向有 1m 可爬障碍。</returns>
        bool TryForwardClimb(bool distanceGated) {
            ...
            // 仅前进输入（按 W）才攀爬：climb 动画沿 Forward，侧向输入（A/D）走原版物理跳
            Vector2? lastWalkOrder = m_componentCreature.ComponentLocomotion.LastWalkOrder;
            if (!lastWalkOrder.HasValue || lastWalkOrder.Value.Y <= ForwardInputThreshold) return false;

            // 前方格方向 = 玩家面向 Forward（climb 动画沿 Forward 直线，检测方向须一致）。
            Vector3 forward = m_componentCreature.ComponentBody.Matrix.Forward;
            Vector2 fwd = new(forward.X, forward.Z);
            if (fwd == Vector2.Zero) return false;
            fwd = Vector2.Normalize(fwd);
            Vector3 fwd3 = new(fwd.X, 0f, fwd.Y);       // Forward 水平单位向量
            ...
            // DDA 沿 Forward 射线遍历穿过的格，reach 内找第一个可爬障碍。
            // 只查射线实际穿过的格：朝障碍角走时射线先穿 Air 再穿障碍（在 reach 内→爬）；
            // 侧向移动时射线不穿侧方障碍（不爬）。step = Forward 各轴方向（±1 或 0）。
            float reach = distanceGated ? ClimbTriggerDistance : 1.0f;
            int stepX = 0, stepZ = 0;
            if (fwd3.X > 1e-3f) stepX = 1; else if (fwd3.X < -1e-3f) stepX = -1;
            if (fwd3.Z > 1e-3f) stepZ = 1; else if (fwd3.Z < -1e-3f) stepZ = -1;
            ...
            for (int i = 0; i <= 4; i++) {
                ...
                if (i == 4) break;
                // 当前格 Forward 出面 t（朝 step 方向的面）
                float tx = float.MaxValue, tz = float.MaxValue;
                if (stepX > 0) tx = (curX + 1 - pos.X) / fwd3.X;
                else if (stepX < 0) tx = (curX - pos.X) / fwd3.X;
                if (stepZ > 0) tz = (curZ + 1 - pos.Z) / fwd3.Z;
                else if (stepZ < 0) tz = (curZ - pos.Z) / fwd3.Z;
